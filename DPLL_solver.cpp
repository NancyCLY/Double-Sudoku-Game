...
int Sat() {
    ...
    int duration;                               //用于表示耗时
                ...
                printf("请输入文件的名称，记得带后缀\n");
                cin >> filename;
                fp = fopen(filename, "r");

                while(fp == NULL) {
                    printf("文件打开失败!请输入正确的文件名,不要忘了后缀！\n ");
                    cin >> filename;
                    fp = fopen(filename, "r");
                } 
                LoadCnf(&S, answer, var_watch, fp);      //调用LoadCnf 函数，读取文件
...
int PrintAnswer(SatAnswer *answer,int result, char filename[100], int duration) {
    cout << "请输入文件的名称以保存文件，不要带后缀，比如输入sat-20即可，而不必附加.res" << endl;
    string loc = "res\\";
    string name;
    cin >> name;
    loc = loc + name;
...
int CheckAnswer(Clause *S, SatAnswer *answer) {//解的正误检查
    FILE *fp;
    Clause *cp = S;
